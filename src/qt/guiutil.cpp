...
{
    // Convert safedeal:// to safedeal:
    //
    //    Cannot handle this later, because safedeal:// will cause Qt to see the part after // as host,
...
{
    return GetAutostartDir() / "safedeal.desktop";
}
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the safedeal app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add safedeal app to startup item list
