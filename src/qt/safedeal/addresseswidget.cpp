...
#include "qt/safedeal/forms/ui_addresseswidget.h"
#include "qt/safedeal/addresslabelrow.h"
#include "qt/safedeal/addnewaddressdialog.h"
#include "qt/safedeal/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(SafeDealGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination sfdAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(sfdAdd, label.toUtf8().constData(),
