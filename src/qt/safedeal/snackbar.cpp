...
#include "qt/safedeal/forms/ui_snackbar.h"
#include "qt/safedeal/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(SafeDealGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &SafeDealGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
