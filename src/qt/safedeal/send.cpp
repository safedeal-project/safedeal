...
#include "qt/safedeal/forms/ui_send.h"
#include "qt/safedeal/addnewcontactdialog.h"
#include "qt/safedeal/qtutils.h"
#include "qt/safedeal/sendchangeaddressdialog.h"
#include "qt/safedeal/optionbutton.h"
#include "qt/safedeal/sendconfirmdialog.h"
#include "qt/safedeal/myaddressrow.h"
...
SendWidget::SendWidget(SafeDealGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-sfd");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination sfdAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(sfdAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(sfdAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
