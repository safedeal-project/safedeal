...
#include "qt/safedeal/topbar.h"
#include "qt/safedeal/forms/ui_topbar.h"
#include "qt/safedeal/lockunlock.h"
#include "qt/safedeal/qtutils.h"
#include "qt/safedeal/receivedialog.h"
...
TopBar::TopBar(SafeDealGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopSFD}, "amount-small-topbar");
    setCssProperty({ui->labelAmountSFD}, "amount-topbar");
    setCssProperty({ui->labelPendingSFD, ui->labelImmatureSFD}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("SafeDeal")
    updateDisplayUnit();
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // SFD Total
    QString totalSFD = GUIUtil::formatBalance(balance, nDisplayUnit);

    // SFD
    // Top
    ui->labelAmountTopSFD->setText(totalSFD);
    // Expanded
    ui->labelAmountSFD->setText(totalSFD);
    ui->labelPendingSFD->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureSFD->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
}
