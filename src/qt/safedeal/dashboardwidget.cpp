...
#include "qt/safedeal/dashboardwidget.h"
#include "qt/safedeal/forms/ui_dashboardwidget.h"
#include "qt/safedeal/sendconfirmdialog.h"
#include "qt/safedeal/txrow.h"
...
DashboardWidget::DashboardWidget(SafeDealGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareSFD, "square-chart-sfd");
    setCssProperty(ui->labelSFD, "text-chart-sfd");
    ...
    setCssProperty(ui->labelAmountSFD, "text-stake-sfd-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &SafeDealGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &SafeDealGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &SafeDealGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("SFD")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            sfd = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalSFD += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesSFD.append(sfd);

        int max = std::max(sfd, sfd);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesSFD);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalSFD > 0) {
        setCssProperty(ui->labelAmountSFD, "text-stake-sfd");
    } else {
        setCssProperty(ui->labelAmountSFD, "text-stake-sfd-disable");
    }
    forceUpdateStyle({ui->labelAmountSFD});
    ui->labelAmountSFD->setText(GUIUtil::formatBalance(chartData->totalSFD, nDisplayUnit));
