...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("SafeDeal client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("SafeDeal"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("SafeDeal"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
