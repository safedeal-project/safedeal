...
UniValue stop(const JSONRPCRequest& jsonRequest)
{
    ...
    if (jsonRequest.fHelp || jsonRequest.params.size() > 1)
        throw std::runtime_error(
            "stop\n"
            "\nStop SafeDeal server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "SafeDeal server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* SFD features */
        {"safedeal", "listmasternodes", &listmasternodes, true },
        {"safedeal", "getmasternodecount", &getmasternodecount, true },
        {"safedeal", "createmasternodebroadcast", &createmasternodebroadcast, true },
        {"safedeal", "decodemasternodebroadcast", &decodemasternodebroadcast, true },
        {"safedeal", "relaymasternodebroadcast", &relaymasternodebroadcast, true },
        {"safedeal", "masternodecurrent", &masternodecurrent, true },
        {"safedeal", "startmasternode", &startmasternode, true },
        ...
        {"safedeal", "mnsync", &mnsync, true },
        {"safedeal", "spork", &spork, true },
...
std::string HelpExampleRpc(std::string methodname, std::string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:62584/\n";
}
