...
*Setup instructions for a gitian build of SafeDeal using a VM or physical system.*

Gitian is the deterministic build process that is used to build the SafeDeal
...
to the SafeDeal GitHub release page.
...
- [Building SafeDeal](#building-safedeal-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/safedeal-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/safedeal-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/safedeal-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/safedeal-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/safedeal-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/safedeal-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/safedeal-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@safedeal.trade) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/safedeal-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/safedeal-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/safedeal-osx-*-build.assert
```
